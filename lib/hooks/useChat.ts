...
export function useChat({
  ...
  const processCommand = async (content: string): Promise<CommandResult> => {
    ...
    if (content.includes('게시판에 올려줘')) {
      ...
      if (content.includes('아이디어 게시판')) {
        boardType = 'idea';
        boardContent = boardContent.replace('아이디어 게시판에 올려줘', '').trim();
      ...
      return {
        type: 'board',
        content: `게시글이 ${boardType === 'idea' ? '아이디어' : '정보'} 게시판에 저장되었습니다.`
      };
    }
    ...
    if (content.includes('추가해줘')) {
      ...
      if (specificDateMatch) {
        ...
        const year = startDate.getFullYear() + (month < startDate.getMonth() ? 1 : 0);
        
        startDate.setFullYear(year, month, day);
        endDate.setFullYear(year, month, day);
        title = title.replace(`${specificDateMatch[1]}월 ${specificDateMatch[2]}일`, '').trim();
      ...
      if (specificTimeMatch) {
        ...
        const adjustedHours = hours <= 11 && content.match(/저녁|오후/) ? hours + 12 : hours;
        
        startDate.setHours(adjustedHours, minutes, 0);
        endDate.setHours(adjustedHours + 1, minutes, 0); // 기본 1시간 duration
        
        // 시간 관련 텍스트 제거
        title = title.replace(/(\d{1,2})시\s*(\d{1,2})?분?/, '').trim()
                    .replace(/(오전|오후|저녁)/, '').trim();
  ...
  const handleGeminiChat = async (
    ...
    try {
      ...
      try {
        geminiService.setModel(options?.model || 'gemini-2.0-flash-exp', systemMessage);
      } catch (error) {
        console.error('Failed to initialize Gemini service:', error);
        throw new Error('Gemini API is not properly configured. Falling back to default model.');
      }
  ...
  const sendMessage = useCallback(
    async (
      ...
      try {
        ...
        if (currentModel.startsWith('gemini-')) {
          ...
          const result = await handleGeminiChat(content, options);
          setIsLoading(false);
          return result;
        }
        ...
        if (commandResult.type !== 'none') {
          const assistantMessage: ChatMessage = { role: 'assistant', content: commandResult.content };
          setMessages(prev => [...prev, assistantMessage]);
          setIsLoading(false);
    ...
    [messages, isLoading, defaultModel, onError]
  );
  ...
  const resetMessages = useCallback((newMessages: any[] = []) => {
